...
```{r}
#Note: options(scipen=999) disables scientific notation for the doc
```
...
```{r}
life_history %>% 
  ggplot(aes(x = gestation, y = wean_mass)) + geom_point(na.rm=T) + scale_y_log10() #scale_y_log10 changes scale of y-axis
```
...
```{r}
life_history %>% 
  ggplot(aes(x=order, y=mass)) + geom_boxplot(na.rm=T) + coord_flip() #coord_flip() flips x- and y-axes
```

Remember, you can always use dplyr functions to focus your question, summarize, etc.
```{r}
life_history %>% 
  filter(order != "Cetacea") %>% #filters out whales cuz they too big
...
```{r}
elephants %>% 
  ggplot(aes(x=sex, fill=sex)) + geom_bar() #fill=sex: add fill color by sex
```
