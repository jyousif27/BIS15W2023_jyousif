...
```{r}
my_palette <- paletteer_d("calecopal::canary")
```
...
```{r}
mass_hra_pal <- paletteer_d("ggpomological::pomological_palette") #mmm yummy apples
homerange %>% 
  ggplot(aes(x=log10.mass, y=log10.hra, color=taxon)) +
  geom_point() +
  scale_color_manual(values = mass_hra_pal)
```
...
homerange %>% 
  ...
  scale_fill_manual(values = mass_hra_pal) +
