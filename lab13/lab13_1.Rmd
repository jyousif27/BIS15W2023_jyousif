...
```
```{r}
names_list <- strsplit(names, split = " .csv") #Removes .csv from end of file name
```
```{r}
...
inner_exampleDF #Must be in both datasets
```
...
left_exampleDF #Must be in first(leftmost) dataset
```
...
right_exampleDF #Must be in last(rightmost) dataset
```

`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID")
full_exampleDF #Must be in either dataset
```
...
anti_exampleDF #Must be in first dataset and not second
```
