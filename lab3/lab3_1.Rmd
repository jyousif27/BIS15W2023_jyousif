...
```{r}
spring_data <- data.frame(name, temp_C = temp, spring)
spring_data
```
...
```{r}
spring_data <- data.frame(scientist = name, temp_C = temp, spring)
names(spring_data)
```
...
spring_data_new <- cbind(spring_data, depth_ft)
spring_data_new
```
...
```{r}
mean(spring_data[ ,2]) #Note: colMeans not used here b/c colMeans must take multiple vectors of data, while mean must take only one set of data
```

5.  Save your hot springs data as a `.csv` file! Do not allow row names.

```{r}
write.csv(spring_data_new, "hot_spring_data.csv", row.names = F)
```
