...
```{r}
hot_springs <- readr::read_csv("hot_spring_data.csv")
```
...
```{r}
class(hot_springs$scientist) #view class
```

```{r}
hot_springs$scientist <- as.factor(hot_springs$scientist) #change to factor
class(hot_springs$scientist) #view new class
```
...
```{r}
summary(fish) #shows info about each vector. for nums, shows stuff like mean, median, etc
#could probably be useful with selectors, like summary(fish[,3]) would show mean/median/quartiles for column 3
```

`glimpse()` is another useful summary function.
```{r}
glimpse(fish) #shows rows, cols, preview of datasets
```
...
```{r}
dim(fish) #total dimensions (rows, cols)
```
...
```{r}
table(fish$lakeid) #shows how many times each lakeid appears in the "lakeid" vector
```

We can also click on the `fish` data frame in the Environment tab or type View(fish).
```{r}
View(fish) #opens new tab to view whole data frame
```
...
table(mammals$Genus) #example of bad data formatting: Genus(caps) instead of genus for just the one vector
```
