...
## 1  4953
```
It makes sense that every entry for Mammalia has an NA for `egg_mass_g`, since mammals don't lay eggs.
...
## 4 Ter        1104     16.3
```
I think that for these variables, an NA means the species in question does not live in that habitat (or just has not been observed in it).
