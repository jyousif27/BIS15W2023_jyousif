...
any_na(amniota)
glimpse(amniota)
#there are NAs but they're shown as -999
```
...
amniota_tidy <- amniota %>% 
  na_if("-999")
#Note: only replace the actual dataset if you are SURE you are replacing a valid NA value
```
...
```{r}
miss_var_summary(life_history) #note: very unlikely for any large dataset to have 0 NAs
```

2. Does this result seem likely? How are NA's represented in the data?
```{r}
glimpse(life_history) #i see -999s
```

3. Replace the placeholder values with NA and recheck.
```{r}
life_hist_naclean <- na_if(life_history, "-999")
miss_var_summary(life_hist_naclean)
```
