...
(1) each variable has its own column?  
   No; there is no column for "drug patient was treated with"
(2) each observation has its own row?  
   No; each row contains four observations, one for each drug
(3) each value has its own cell?  
   Yes
...
plant_data %>% 
  pivot_longer(-c(genotype, water_sched_prog, greenhouse), 
               names_to = "day", 
               names_prefix = "day", #this leaves us with "day: 1, 2, 3" instead of "day: day1, day2, day3"
               values_to = "v1", 
               values_drop_na = T)
```
