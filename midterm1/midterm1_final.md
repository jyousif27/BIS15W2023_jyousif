...
## 1 19
```
There are 19 different orders represented in the data.
...
The Woodland habitat has the highest number of unique species in it, at 177 species.
...
The species with the largest winter range is *Puffinus griseus*. The species with the smallest winter range is *Alauda arvensis*.
...
ducks %>% 
  ...
  select(scientific_name, common_name, habitat)
```
...
##   <chr>                <chr>        <chr>  
## 1 Somateria mollissima Common Eider Ocean
```

The common eider, *Somateria mollissima*, lives in (on?) the ocean.
...
```r
ducks %>% 
  select(common_name, scientific_name, log10_mass, migratory_strategy) %>% 
...
Birds which migrate further tend to have a smaller body mass. The exception is that moderate-distance migrators have a higher average body mass than short-distance migrators.
...
```r
eagles <- eagles %>% 
  mutate(conservation_status = if_else(population_size < 250000, "Threatened", "Safe"))
eagles
```
...
```r
eagles %>% 
  filter(is.na(population_size) == T)
```
...
Yes. These species have an "NA" in the field for population size, meaning that we don't know their population size (and thus whether they are threatened or not).

Problem 12. (4 points) Use the `ecosphere` data to perform one exploratory analysis of your choice. The analysis must have a minimum of three lines and two functions. You must also clearly state the question you are attempting to answer.

**Question:** Do birds with shorter life expectancies have more offspring? (How does life expectancy associate with mean eggs per clutch and mean age at sexual maturity?)


```r
ecosphere %>% 
  select(contains("name"), life_expectancy, mean_eggs_per_clutch, mean_age_at_sexual_maturity) %>% 
  group_by(life_expectancy) %>% 
  summarize(mean_eggs_per_clutch = mean(mean_eggs_per_clutch), mean_age_at_maturity = mean(mean_age_at_sexual_maturity))
```

```
## # A tibble: 3 × 3
##   life_expectancy mean_eggs_per_clutch mean_age_at_maturity
##   <chr>                          <dbl>                <dbl>
## 1 Long                            3.22                 3.60
## 2 Middle                          4.46                 1.68
## 3 Short                           4.82                 1.16
...
As expected, birds with a lower life expectancy have a lower average age at sexual maturity and a higher average number of eggs laid per clutch.
